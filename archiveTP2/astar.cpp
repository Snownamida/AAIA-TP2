...
using PriorityQueue =
    priority_queue<pair<State, int>, vector<pair<State, int>>, compare>;
using HashMap = unordered_map<State, Info, State::hash>;

void printSolution(StateGraph &g, const State &s0, const State &s,
                   const HashMap &map) {
...
void astar(const State &s0, StateGraph &g) {
  ...
  while (!q.empty()) {
    ...
    q.pop();
    if (!map[s].isGrey)
      continue; // s is ignored because it is black
    nbIter++;
    if (g.isFinal(s)) {
      printf("Optimal solution of length %d found in %d iterations and %f "
             "seconds\n",
  ...
  printf("The problem has no solution (number of iterations = %d; CPU time = "
         "%fs)\n",
